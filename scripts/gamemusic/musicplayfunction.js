...
export function musicSystemTick() {
    try {
        ...
        for (const [playerId, state] of playerMusicState.entries()) {
        ...
        for (const player of world.getAllPlayers()) {
            ...
            if (playerMusicState.has(player.id)) {
                continue;
            }
            ...
            for (const musicId in MUSIC_DATA) {
                ...
                if (player.hasTag(musicInfo.id)) {
                    ...
                    // 이 함수가 2틱마다 1씩 쿨다운을 감소시키기 때문입니다.
                    const cooldownValue = Math.ceil(musicInfo.durationTicks);

                    // 플레이어의 상태를 Map에 기록합니다.
                    playerMusicState.set(player.id, {
                    ...
                    // 한 플레이어에게는 한 곡만 재생해야 하므로, 안쪽 루프를 탈출합니다.
                    break;
                }
            }
        }
    ...
    for (const player of players) {
        ...
        const cooldownValue = playerMusicState.has(player.id) ? playerMusicState.get(player.id).cooldown : 0;

        // 플레이어에게 현재 태그와 쿨다운 상태를 actionbar로 표시합니다.
